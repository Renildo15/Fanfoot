    ...
    with open("./data/coach_csv.csv", "r", encoding="utf-8") as f:
        ...
        for coach_data in csv_data:
            try:
                ...
                if not club:
                    print(
                        f"Clube com ID {club_id} não encontrado. Pulando técnico {coach_data['full_name']}"
                    )
                    continue
                ...
                if not country:
                    print(
                        f"País com ID {country_id} não encontrado. Pulando técnico {coach_data['full_name']}"
                    )
                    continue
                ...
                try:
                    ...
                    print(
                        f"Estilo inválido: {coach_data['style']}. Usando padrão BALANCED"
                    )
                ...
                print(
                    f"Erro ao processar técnico {coach_data.get('full_name', 'N/A')}: {e}"
                )
                continue
